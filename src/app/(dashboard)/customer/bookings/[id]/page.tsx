    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3 mt-2 text-sm text-muted-foreground">
          ...
          {(event as { city: string | null }).city && <span className="flex items-center gap-1.5"><MapPin className="size-4" />{(event as { city: string }).city}</span>}
      ...
      {booking.status === "INQUIRY" && (
        <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{vendor.businessName} typically responds within {vendor.responseTime} hours.</p>
          </div>
        </div>
      )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 space-y-1 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {vendor.isVerified && <span className="text-[11px] font-semibold text-green-600 flex items-center gap-1"><CheckCircle className="size-3" /> Verified</span>}
                  </div>
                  <p className="text-sm text-muted-foreground">{VENDOR_CATEGORY_LABELS[vendor.category] ?? vendor.category} &middot; {vendor.city}, {vendor.state}</p>
              ...
              {payments.length === 0 ? (
                ...
                <div key={p.id} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <Badge variant={p.status === "PAID" ? "success" : p.status === "FAILED" ? "destructive" : "warning"} className="text-[10px]">{p.status}</Badge>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {!["CANCELLED","COMPLETED","DISPUTED"].includes(booking.status) && (
                <Button variant="outline" size="sm" className="w-full text-destructive hover:text-destructive hover:bg-destructive/10">Cancel booking</Button>
              )}
