      ...
      <CardContent className="space-y-3">
        {payments.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            Payment schedule will appear once the quote is accepted.
          </p>
        ) : (
          payments.map((p) => (
            <div key={p.id} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                ...
                <Badge variant={PAYMENT_BADGE_VARIANT[p.status] ?? "warning"} className="text-[10px]">
                  {p.status}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                ...
                {p.paidAt && (
                  <span className="text-[10px] text-muted-foreground">{formatDate(p.paidAt)}</span>
                )}
              </div>
              {p.status === "PENDING" && (
                <Button size="sm" className="w-full mt-1 h-8 text-xs" asChild>
                  <Link href={`/customer/bookings/${bookingId}/payments`}>Pay now</Link>
                </Button>
              )}
            </div>
          ))
